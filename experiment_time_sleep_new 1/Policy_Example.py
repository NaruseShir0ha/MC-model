...
def get_Policy(Obj_Name, Se, Sa, task_pool,scene):
    # 各种机器的策略分配  wp: 下边这些1,3等数字代表什么? 代表安全等级，即重要性
    object_data = pd.read_excel('data/'+scene+'/object.xlsx')
    arrange = {}
    for i in range(len(object_data)):
        arrange[object_data.iloc[i, 0]] = {}
        # 将object_data.iloc[i,3]用/分隔开
        for item in object_data.iloc[i, 3].split('/'):
            policy = item.split(':')[0]
            level = item.split(':')[1]
            arrange[object_data.iloc[i, 0]][policy] = int(level)
